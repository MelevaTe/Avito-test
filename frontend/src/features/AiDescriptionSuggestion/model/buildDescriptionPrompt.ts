...
export const buildDescriptionPrompt = ({
  ...
  if (hasDescription) {
    return [
      ...
      "Сохрани суть, улучши стиль.",
      "Ответ на русском языке, до 500 символов.",
      "Верни только готовый текст описания.",
      "Не используй markdown, заголовки, символы #, списки, кавычки-елочки и пояснения от себя.",
    ]
      .filter(Boolean)
      .join("\n");
  }
  ...
  return [
    ...
    paramsStr ? `Параметры: ${paramsStr}` : "",
    "Описание должно быть на русском языке, до 500 символов.",
    "Верни только готовый текст описания.",
    "Не используй markdown, заголовки, символы #, списки, кавычки-елочки и пояснения от себя.",
  ]
    .filter(Boolean)
